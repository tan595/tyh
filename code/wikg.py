...
class WiKG(nn.Module):
    def __init__(self, dim_in=384, dim_hidden=512, topk=6, n_classes=2, agg_type='bi-interaction', dropout=0.3, pool='attn'):
        ...
        self.W_head = nn.Linear(dim_in, dim_hidden)
        self.W_tail = nn.Linear(dim_in, dim_hidden)
    ...
    def forward(self, x, e_h, e_t):
        ...
        # ka_weight = torch.einsum('ikl,ikm->ik', Nb_h, gate)
        ...
        if self.agg_type == 'gcn':
            ...
            sum_embedding = self.activation(self.linear1(x + e_Nh))
            bi_embedding = self.activation(self.linear2(x * e_Nh))
